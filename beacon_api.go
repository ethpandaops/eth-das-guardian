...
const (
	...
	GloasForkScheduleIdx = 7                // Gloas is the 8th fork -> index = 7
)
...
func (b *BeaconAPIImpl) Init(ctx context.Context) error {
	...
	if log.GetLevel() >= log.DebugLevel {
		...
		for _, key := range []string{"GENESIS_VALIDATORS_ROOT", "GENESIS_FORK_VERSION", "DENEB_FORK_EPOCH", "ELECTRA_FORK_EPOCH", "FULU_FORK_EPOCH", "GLOAS_FORK_EPOCH", "GLOAS_FORK_VERSION"} {
	...
	gloasForkEpoch := uint64(math.MaxInt)
	if len(forkSchedules.Data) > GloasForkScheduleIdx {
		gloasForkEpoch, err = strconv.ParseUint(forkSchedules.Data[GloasForkScheduleIdx].Epoch, 10, 64)
		if err != nil {
			return err
		}
	}
...
func (b *BeaconAPIImpl) GetForkDigest(slot uint64) ([]byte, error) {
	...
	// isPeerDASActive gates the BPO fork-digest XOR path. It is true from
	// Fulu onwards (PeerDAS + BPO), including Gloas.
	var isPeerDASActive bool
	var currentBlobParams *BlobScheduleEntry

	if forkEpoch, ok := b.specs["GLOAS_FORK_EPOCH"].(uint64); ok && currentEpoch >= forkEpoch {
		forkVersion = b.specs["GLOAS_FORK_VERSION"].(phase0.Version)
		isPeerDASActive = true
	} else if forkEpoch, ok := b.specs["FULU_FORK_EPOCH"].(uint64); ok && currentEpoch >= forkEpoch {
	...
	if isPeerDASActive {
		...
		if log.GetLevel() >= log.DebugLevel {
			...
			}).Debug("Initial BPO parameters set for PeerDAS")
		}
	...
	if log.GetLevel() >= log.DebugLevel {
		b.cfg.Logger.WithFields(log.Fields{
			...
			"is_peerdas_active":       isPeerDASActive,
...
func (b *BeaconAPIImpl) GetGloasForkEpoch() uint64 {
